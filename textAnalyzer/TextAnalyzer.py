...
if __name__ == "__main__":
    ...
    if args.mode=='TOP':
        ...
        words=lines.flatMap(lambda s: s.split()) \
             ...
             .filter(lambda word: word[0]!='')\
             .reduceByKey(lambda x, y: x + y) \
             .takeOrdered(20, lambda (key, value): -value)
        f=open(sys.argv[3], 'w')
        for x in words:
                f.write("(%s, %f)\n" % x)
        f.close()
    ...
    if args.mode=='IDF':
        ...
        # are removed
        alltext=sc.wholeTextFiles(sys.argv[2])
        doc=alltext.count()
        idf_word=alltext.flatMapValues(lambda s : s.split())\
                        .map(lambda word: (stripNonAlpha(toLowerCase((word[1])), word[0]))\
                        .filter(lambda word: word[0]!= '')\
                        .distinct()\
                        .map(lambda word: (word[0], 1))\
                        .reduceByKey(lambda x, y:x + y)\
                        .map(lambda word: (word[0], np.log(doc/word[1])))\
                        .saveAsTextFile(sys.argv[3])
        pass
    ...
    if args.mode=='SIM':
        ...
        # strings representing pairs of the form (TERM,VAL),
        # where TERM is a lowercase, letter-only string and VAL is a numeric value.
        tfidfa=sc.textFile(args.input).map(eval)
        tfidfb=sc.textFile(args.other).map(eval)
        numer=tfidfa.join(tfidfb).map(lambda x: x[1][0] * x[1][1]).reduce(lambda x, y: x + y)
        deno=np.sqrt(tfidfa.map(lambda x: x[1] * x[1]).reduce(lambda x, y: x + y)*tfidfb.map(lambda x: x[1] * x[1]).reduce(lambda x, y: x + y))
        cos = numer/deno
        f = open(args.output, 'w')
        f.write("%f" % cos)
        f.close()
