...
def joinAndPredictAll(R,U,V,N):
    ...
    """
    predict = R.map(lambda(i,j,rij):(i,(j,rij))).join(U,numPartitions=N)\
...
def SE(joinedRDD):
    ...
    """
    return joinedRDD.map(lambda(i,j,delta,ui,vj):delta**2)\
...
def normSqRDD(profileRDD,param):
    """ Receives as input an RDD of profiles (e.g., U) as well as a parameter (e.g., λ) and computes the square of norms:
        ...
        λ Σ_i ||ui||_2^2         
    """
    normsq=profileRDD.map(lambda(i,u):np.dot(u,u)).reduce(lambda x,y:x+y)
    return param*normsq
...
def adaptU(joinedRDD,gamma,lam,N):
    ...
    ui=ui.mapValues(lambda (ui,grad):ui-gamma*(grad+2*lam*ui))		
    return ui   
...
def adaptV(joinedRDD,gamma,mu,N):
    ...
    vj=joinedRDD.map(lambda (i,j,delta,ui,vj):(j,(vj,gradient_v(delta,ui,vj))))\                .reduceByKey(lambda x,y:(x[0],x[1]+y[1]),numPartitions=N)
    vj=vj.mapValues(lambda (vj,grad):vj-gamma*(grad+2*mu*vj))
    return vj
